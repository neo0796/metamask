...
const Navbar = () => {
  ...
  const [balance, setBalance] = useState<string>('0 MM'); // Set initial balance
  ...
  const connectWallet = async () => {
    if (window.ethereum) {
      try {
        ...
        setBalance('15 MM'); // Replace this with your logic to fetch balance
      } catch (error) {
  ...
  return (
    <nav className="absolute top-0 left-0 right-0 flex justify-between items-center p-6 bg-transparent z-50">
      <div 
        ref={logoRef} 
        className="flex items-center font-bold text-3xl text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-orange-600"
      >
        <a href="/">MetaMaster</a>
      </div>
      ...
      <div className="flex items-center space-x-4">
        ...
        <button 
          ...
          className="text-white bg-gradient-to-r from-orange-500 to-orange-700 hover:from-orange-400 hover:to-orange-600 transition duration-300 px-4 py-2 rounded-lg transform hover:scale-105"
        >
